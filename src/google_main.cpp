...
int main() {
        ...
        if (programState->ImGuiEnabled) {
                glfwSetInputMode(
                    window, GLFW_CURSOR,
                    GLFW_CURSOR_HIDDEN);  // GLFW_CURSOR_DISABLED//NORMAL
        }
        // Init Imgui
        IMGUI_CHECKVERSION();
        ImGui::CreateContext();
        ...
        // -------------------------
        Shader ourShaderModel("resources/shaders/model_lighting.vs",
                              "resources/shaders/model_lighting.fs");
        Shader ourShaderBase("resources/shaders/vertex_shader.vs",
        ...
        unsigned int diffuseMap = loadTexture(
            FileSystem::getPath("resources/textures/stone_diffuse.jpg").c_str(),
            false);
        unsigned int specularMap = loadTexture(
            FileSystem::getPath("resources/textures/stone_specular.jpg")
                .c_str(),
            false);
        ...
        while (!glfwWindowShouldClose(window)) {
                ...
                // model = glm::rotate_slow(model, (float)glfwGetTime()/10,
                ...
                ourShaderModel.setVec3("pointLight.position",
                                       pointLight.position);
                ourShaderModel.setVec3("pointLight.ambient",
                                       pointLight.ambient);
                ourShaderModel.setVec3("pointLight.diffuse",
                                       pointLight.diffuse);
                ourShaderModel.setVec3("pointLight.specular",
                                       pointLight.specular);
                ourShaderModel.setFloat("pointLight.constant",
                                        pointLight.constant);
                ourShaderModel.setFloat("pointLight.linear", pointLight.linear);
                ourShaderModel.setFloat("pointLight.quadratic",
                                        pointLight.quadratic);
                ourShaderModel.setVec3("viewPosition",
                                       programState->camera.Position);
                ourShaderModel.setFloat(
                    "material.shininess",
                    128.0f);  // 256 tackasto, manja vrednost rasipanje
                ourShaderModel.setBool("gamma", gamma_t);
                // body
                view = programState->camera.GetViewMatrix();
                glm::mat4 modelPotato = glm::mat4(1.0f);
                // modelPotato = glm::rotate_slow(modelPotato,
                // (float)glfwGetTime()/10, glm::vec3(0.0,1.0,0.0));
                modelPotato = glm::rotate(modelPotato, glm::radians(-90.0f),
                                          glm::vec3(0.0, 1.0, 0.0));
                modelPotato =
                    glm::translate(modelPotato, glm::vec3(0, 0.16, 0));
                ...
                skyboxShader.use();
                view =
                    glm::mat4(glm::mat3(programState->camera.GetViewMatrix()));
...
unsigned int loadTexture(char const *path, bool gammaCorrection) {
        ...
        if (data) {
                ...
                if (nrComponents == 1) {
                        ...
                        internalFormat =
                            gammaCorrection ? GL_SRGB_ALPHA : GL_RGBA;
                        dataFormat = GL_RGBA;
                }
